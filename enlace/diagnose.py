...
class DiagnosticReport:
    ...
    def format_text(self) -> str:
        ...
        lines.append(
            f"Backend:    {'yes' if self.has_backend else 'no'}"
            + (f" ({self.backend_framework})" if self.backend_framework else "")
        )
        lines.append(
            f"Frontend:   {'yes' if self.has_frontend else 'no'}"
        ...
        for sev in (Severity.CRITICAL, Severity.MEDIUM, Severity.LOW, Severity.INFO):
            ...
            for issue in sev_issues:
                ...
                if issue.breaks_standalone:
                    lines.append(
                        "    WARNING: This fix would break standalone operation."
                    )
                lines.append("")
...
def _scan_python_for_hardcoded_ports(source: str, rel: str, report: DiagnosticReport):
    """Detect hardcoded port numbers in uvicorn.run() calls."""
    for i, line in enumerate(source.splitlines(), 1):
        if (
            "uvicorn.run" in line
            or "uvicorn.run" in source[max(0, source.find(line) - 200) :]
        ):
...
def _scan_python_for_hardcoded_urls(source: str, rel: str, report: DiagnosticReport):
...
def _scan_js_for_hardcoded_urls(source: str, rel: str, report: DiagnosticReport):
...
def _scan_js_for_websocket(source: str, rel: str, report: DiagnosticReport):
    ...
    ws_pattern = re.compile(r"""new\s+WebSocket\s*\(|\.connect\s*\(\s*['"`]ws[s]?://""")
    for i, line in enumerate(source.splitlines(), 1):
...
def _check_env_files(app_dir: Path, report: DiagnosticReport):
    ...
    for env_name in (".env", ".env.local", ".env.development", ".env.production"):
        for root in [app_dir] + [
            app_dir / d for d in ("frontend", "backend", "client")
        ]:
...
def _check_html_for_issues(app_dir: Path, report: DiagnosticReport):
    ...
    html_files = [
        f for f in html_files if "node_modules" not in str(f) and ".next" not in str(f)
    ]
